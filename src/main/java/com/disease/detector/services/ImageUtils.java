...
public class ImageUtils
{
  ...
  private static final boolean equals(final int[] data1, final int[] data2)
  {
    ...
    for (int i = 0; i < length; i++)
    {
      if (data1[i] != data2[i])
      {
        ...
        {
          logger.debug("Both pixles at spot {} are different but 100% transparent.",
              Integer.valueOf(i));
        }
        else
        {
  ...
  private static final int[] getPixels(final BufferedImage img, final File file)
  {
    ...
    {
      pixelImg = new ColorConvertOp(ColorSpace.getInstance(ColorSpace.CS_sRGB), null)
          .filter(img, null);
    }
  ...
  {
    return equals(getPixels(getBufferedImage(file1), file1),
        getPixels(getBufferedImage(file2), file2));
  }
  ...
  public final static boolean visuallyCompare(final File file1, final File file2)
  {
    ...
    {
      logger.info(
          "The files \"{}\" and \"{}\" are not pixel by pixel the same image. Manual comparison "
              + "required.",
